...
const continentCountDiv = document.getElementById("continentCount");
...
// Update Continent...
function updateContinentCount(data) {
  const count = {};
  data.forEach(c => {
    count[c.continent] = (count[c.continent] || 0) + 1;
  });

  for (let continent in count) {
    continentCountDiv.innerHTML += `<p>${continent}: <b>${count[continent]}</b></p>`;
  }
}
